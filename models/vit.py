...
from .model_utils import auto_get_activations, auto_project_weights, forward_cache_activations
...
class EncoderBlock(nn.Module):
    ...
    def get_activations(self, input,  block_key, prev_recur_proj_mat = None):      
        ...
        if prev_recur_proj_mat is None:
            act["pre"][f"{block_key}.self_attn.qkv"] = deepcopy(x.clone().detach().view(-1,  x.shape[-1]).cpu().numpy())
            q, k, v = F._in_projection_packed(x, x, x, self.self_attention.in_proj_weight, self.self_attention.in_proj_bias)
            act["post"][f"{block_key}.self_attn.query"] = deepcopy(q.clone().detach().view(-1,  q.shape[-1]).cpu().numpy())
            act["post"][f"{block_key}.self_attn.key"] = deepcopy(k.clone().detach().view(-1,  k.shape[-1]).cpu().numpy())
            act["post"][f"{block_key}.self_attn.value"] = deepcopy(v.clone().detach().view(-1,  v.shape[-1]).cpu().numpy())
            q = q.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            k = k.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            v = v.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            attn_output = F.scaled_dot_product_attention(q,k,v)
            attn_output = attn_output.permute(0, 2, 1, 3).contiguous().view(seq_len*bsz, embed_dim)
            act["pre"][f"{block_key}.self_attn.out_proj"] = deepcopy(attn_output.clone().detach().view(-1,  attn_output.shape[-1]).cpu().numpy())
            attn_output = F.linear(attn_output, self.self_attention.out_proj.weight, self.self_attention.out_proj.bias)
            act["post"][f"{block_key}.self_attn.out_proj"] = deepcopy(attn_output.clone().detach().view(-1,  attn_output.shape[-1]).cpu().numpy())
        else:
            act["pre"][f"{block_key}.self_attn.qkv"] = deepcopy(x.clone().detach().view(-1,  x.shape[-1]).cpu().numpy())
            x = torch.matmul( x.view(-1,  x.shape[-1]), prev_recur_proj_mat["pre"][f"{block_key}.self_attn.qkv"] ).view_as(x)
            q, k, v = F._in_projection_packed(x, x, x, self.self_attention.in_proj_weight, self.self_attention.in_proj_bias)
            act["post"][f"{block_key}.self_attn.query"] = deepcopy(q.clone().detach().view(-1,  q.shape[-1]).cpu().numpy())
            act["post"][f"{block_key}.self_attn.key"] = deepcopy(k.clone().detach().view(-1,  k.shape[-1]).cpu().numpy())
            act["post"][f"{block_key}.self_attn.value"] = deepcopy(v.clone().detach().view(-1,  v.shape[-1]).cpu().numpy())
            q = torch.matmul( q.view(-1,  q.shape[-1]), prev_recur_proj_mat["pre"][f"{block_key}.self_attn.query"] ).view_as(q)
            k = torch.matmul( k.view(-1,  k.shape[-1]), prev_recur_proj_mat["pre"][f"{block_key}.self_attn.key"] ).view_as(k)
            v = torch.matmul( v.view(-1,  v.shape[-1]), prev_recur_proj_mat["pre"][f"{block_key}.self_attn.value"] ).view_as(v)
            q = q.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            k = k.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            v = v.view(bsz, seq_len, self.num_heads, embed_dim//self.num_heads).permute(0,2,1,3)
            attn_output = F.scaled_dot_product_attention(q,k,v)
            attn_output = attn_output.permute(0, 2, 1, 3).contiguous().view(seq_len*bsz, embed_dim)
            act["pre"][f"{block_key}.self_attn.out_proj"] = deepcopy(attn_output.clone().detach().view(-1,  attn_output.shape[-1]).cpu().numpy())
            attn_output = torch.matmul( attn_output.view(-1,  attn_output.shape[-1]), prev_recur_proj_mat["pre"][f"{block_key}.self_attn.out_proj"] ).view_as(attn_output)
            attn_output = F.linear(attn_output, self.self_attention.out_proj.weight, self.self_attention.out_proj.bias)
            act["post"][f"{block_key}.self_attn.out_proj"] = deepcopy(attn_output.clone().detach().view(-1,  attn_output.shape[-1]).cpu().numpy())
            attn_output = torch.matmul( attn_output.view(-1,  attn_output.shape[-1]), prev_recur_proj_mat["post"][f"{block_key}.self_attn.out_proj"] ).view_as(attn_output)
        ...
        for layer in self.mlp:            
            if isinstance(layer, nn.Linear):
                ...
                layer_name = ""
            layer_acts, y_reshaped  = forward_cache_activations(y.view(-1,  y.shape[-1]), layer, layer_name, prev_recur_proj_mat)
    ...
    def project_weights(self,  projection_mat_dict, block_key):
        ...
        w_q.data = torch.mm(projection_mat_dict["post"][f"{block_key}.self_attn.query"].transpose(0,1) , torch.mm(w_q.data, projection_mat_dict["pre"][f"{block_key}.self_attn.qkv"].transpose(0,1) )).view_as(w_q.data)
        w_k.data = torch.mm(projection_mat_dict["post"][f"{block_key}.self_attn.key"].transpose(0,1) , torch.mm(w_k.data, projection_mat_dict["pre"][f"{block_key}.self_attn.qkv"].transpose(0,1) )).view_as(w_k.data)
        w_v.data = torch.mm(projection_mat_dict["post"][f"{block_key}.self_attn.value"].transpose(0,1) , torch.mm(w_v.data, projection_mat_dict["pre"][f"{block_key}.self_attn.qkv"].transpose(0,1) )).view_as(w_v.data)
        ...
        self.self_attention.out_proj.weight.data = torch.mm(projection_mat_dict["post"][f"{block_key}.self_attn.out_proj"].transpose(0,1) , torch.mm(self.self_attention.out_proj.weight.data, projection_mat_dict["pre"][f"{block_key}.self_attn.out_proj"].transpose(0,1) )).view_as(w_v.data)
        if self.self_attention.out_proj.bias is not None:
            self.self_attention.out_proj.bias.data  = torch.mm(self.self_attention.out_proj.bias.data.unsqueeze(0),projection_mat_dict["post"][f"{block_key}.self_attn.out_proj"]).squeeze(0)
        mlp_ind = 0
        for layer in self.mlp:
            if isinstance(layer, nn.Linear):
                layer.weight.data = torch.mm(projection_mat_dict["post"][f"{block_key}.mlp.linear{mlp_ind}"].transpose(0,1) ,torch.mm(layer.weight.data.flatten(1), projection_mat_dict["pre"][f"{block_key}.mlp.linear{mlp_ind}"].transpose(0,1))).view_as(layer.weight.data)
                if layer.bias is not None:
                    layer.bias.data  = torch.mm(layer.bias.data.unsqueeze(0),projection_mat_dict["post"][f"{block_key}.mlp.linear{mlp_ind}"]).squeeze(0)
                mlp_ind+=1
        return
...
class Encoder(nn.Module):
    ...
    def get_activations(self, input, prev_recur_proj_mat = None):      
        act={"pre":OrderedDict(), "post":OrderedDict()}
        encoderblock_ind=0
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        input = input + self.pos_embedding
        ...
        for layer in self.layers:
            ...
            encoderblock_ind+=1
            layer_acts, input = layer.get_activations(input, block_name, prev_recur_proj_mat)
            act["pre"].update(layer_acts["pre"])
            act["post"].update(layer_acts["post"])     
...
class VisionTransformer(nn.Module):
    ...
    def get_activations(self, x, prev_recur_proj_mat = None):      
        ...
        # x = self.conv_proj(x)
        if isinstance(self.conv_proj, nn.Conv2d):
            act, x  = forward_cache_activations(x, self.conv_proj, f"conv_proj.conv{conv_proj_ind}", prev_recur_proj_mat)  
            conv_proj_ind+=1
        else:
            act={"pre":OrderedDict(), "post":OrderedDict()}
            for layer in self.conv_proj:
                if isinstance(layer, nn.Conv2d):
                    ...
                    layer_name = ""
                layer_acts, x  = forward_cache_activations(x, layer, f"conv_proj.conv{conv_proj_ind}", prev_recur_proj_mat)  
                act["pre"].update(layer_acts["pre"])  
                act["post"].update(layer_acts["post"])  
        ...
        encoder_activation, x = self.encoder.get_activations(x, prev_recur_proj_mat)
        act["pre"].update(encoder_activation["pre"])  
        act["post"].update(encoder_activation["post"])  

        # Classifier "token" as used by standard language architectures
        x = x[:, 0]

        if isinstance(self.heads, nn.Linear):    
            layer_acts, x  = forward_cache_activations(x, self.heads, f"heads.linear{heads_ind}", prev_recur_proj_mat)  
            act["pre"].update(layer_acts["pre"])  
            act["post"].update(layer_acts["post"])    
            heads_ind+=1
        else:
            for layer in self.heads:
                if isinstance(layer, nn.Linear):
                    ...
                    layer_name = ""
                layer_acts, x  = forward_cache_activations(x, layer, layer_name, prev_recur_proj_mat)  
                act["pre"].update(layer_acts["pre"])  
                act["post"].update(layer_acts["post"]) 
    ...
    def project_weights(self, projection_mat_dict, proj_classifier=False):
        ...
        if isinstance(self.heads, nn.Linear):
            ...
            for layer in self.heads:
                if isinstance(layer, nn.Linear):
                    if (heads_ind == self.max_head_linear_layers-1) and (not proj_classifier):
